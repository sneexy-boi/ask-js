...
export class Migration1740343668377 implements MigrationInterface {
	...
	public async up(queryRunner: QueryRunner): Promise<void> {
		await queryRunner.query(
			`CREATE TABLE "user" ("id" character varying NOT NULL, "password" character varying NOT NULL, CONSTRAINT "PK_cace4a159ff9f2512dd42373760" PRIMARY KEY ("id"))`
		);
		await queryRunner.query(
			`CREATE TABLE "auth" ("id" character varying NOT NULL, "token" character varying NOT NULL, "user" character varying NOT NULL, "createdAt" character varying NOT NULL, CONSTRAINT "PK_7e416cf6172bc5aec04244f6459" PRIMARY KEY ("id"))`
		);
		await queryRunner.query(
			`CREATE TABLE "ask" ("id" character varying NOT NULL, "used" boolean NOT NULL DEFAULT false, "code" character varying NOT NULL, "creator" character varying NOT NULL, "createdAt" character varying NOT NULL, CONSTRAINT "PK_d5060cd0ad3d8db27dc0ec98a86" PRIMARY KEY ("id"))`
		);
		...
		await queryRunner.query(
			`ALTER TABLE "user" ADD "prompt" character varying`
		);
		await queryRunner.query(
		...
		await queryRunner.query(
			`ALTER TABLE "ask" ADD "to" character varying NOT NULL`
		);
		await queryRunner.query(`ALTER TABLE "ask" ADD "cw" character varying`);
		await queryRunner.query(
			`ALTER TABLE "ask" ADD "content" character varying NOT NULL`
		);
		await queryRunner.query(
			`ALTER TABLE "ask" ADD "nickname" character varying`
		);
	}
